...
def water_color_effect(decode_array_to_img):
    wc = cv2.stylization(decode_array_to_img, sigma_s=60, sigma_r=0.6)

    status, output_img = cv2.imencode('.PNG', wc)

    return output_img

def invert(decode_array_to_img):
    inv = cv2.bitwise_not(decode_array_to_img)

    status, output_img = cv2.imencode('.PNG', inv)

    return output_img

def hdr(decode_array_to_img):
    hd = cv2.detailEnhance(decode_array_to_img, sigma_s=12, sigma_r=0.15)

    status, output_img = cv2.imencode('.PNG', hd)
